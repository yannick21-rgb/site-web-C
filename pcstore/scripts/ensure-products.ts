...
const PRODUCTS: {
  ...
  },
  {
    ...
    category: "BUREAUTIQUE",
    shortDescription:
      "Le PC d'entrée de gamme qui fait bien son travail : bureautique, navigation et vidéos. Simple et fiable.",
    price: 199_000,
    stock: 5,
    cpu: "Ryzen 3 7320U",
    gpu: "AMD Radeon",
...
async function main() {
  ...
  if (count > 0) {
    console.log(`[ensure-products] ${count} produit(s) déjà présent(s) : rien à faire.`);
    return;
  }
  ...
  console.log(`[ensure-products] ${PRODUCTS.length} produits insérés (catalogue initial).`);
}
